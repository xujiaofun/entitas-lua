...
function M:matches(entity)
    local all_cond = not self._all or entity:has(table_unpack(self._all))
    local any_cond = not self._any or entity:has_any(table_unpack(self._any))
    local none_cond = not self._none or not entity:has_any(table_unpack(self._none))
